...
// PUT recibiendo un parametro por id y actualizar datos del cliente
app.put("/clientes/:id",(req,res)=>{
    ...
    // ya con el Id se busca el index del cliente a actualizar
    const clienteIndex =  clientes.findIndex(cliente => cliente.id === id)
    // si lo encuentra retorna  el numero del indice del cliente
    // si no lo encuentra genera un   -1 que significa que no es posible
    if(clienteIndex !== -1){
        // si el cliente existe  y  lo encuentro  lo actualizo
        clientes[clienteIndex].nombre = nombre
        clientes[clienteIndex].apellido = apellido
        // se verifica por consola que todo se actualizo
        console.log(clientes)
        res.send({status: "success",mensaje:"cliente actualizado"})
    }else{
        res.status(404).send({status: "error",mensaje:"cliente no encontrado"})
        console.log('cliente no encontrado, escribe  bien')
    }
})
...
app.delete("/clientes/:id",(req,res)=>{
    ...
    if(clienteIndex !== -1){
        // si el cliente no existe lo elimino 
